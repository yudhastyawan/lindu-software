...
def iterbending3(niter,cacah,x,y,vxyz,xs,ys,xr,yr,dvx,dvy):
    xi = [xr, xs]
    yi = [yr, ys]
    ...
    while (j < niter and ((np.min(yi) >= np.min(x) and np.max(yi) <= np.max(y)) and (np.min(xi) >= np.min(y) and np.max(xi) <= np.max(x)))):
        # print(j)
        # print(min(xi), max(xi), min(yi), max(yi))
        xa = xi
        ya = yi
        print(min(xa), max(xa), min(ya), max(ya))
        ...
        # if (np.min(yb) >= 0 and np.max(yb) <= np.max(y)) and (np.min(xb) >= 0 and np.max(xb) <= np.max(x)):
        xi = xb
        yi = yb
        # else:
        #     xi = xa
...
def iterbending2(niter,cacah,x,y,vxyz,xs,ys,xr,yr,dvx,dvy):
    ...
    for j in range(niter):
        ...
        ya = yi
        # print(min(xi), max(xi), min(yi), max(yi))
        if ((np.min(yi) >= np.min(y) and np.max(yi) <= np.max(y)) and (np.min(xi) >= np.min(x) and np.max(xi) <= np.max(x))):
            for i in range(cacah - 1):
                x1 = xi[i]
                y1 = yi[i]
                x2 = xi[i + 2]
...
# def fwdModel(srcfile,statfile,velfile,evtfile,rayfile,veloutfile,gr,biter,cacah,pert,progress_callback):
def fwdModel(srcfile,statfile,velfile,evtfile,rayfile,veloutfile,gr,biter,cacah,pert,progress_callback):
...
srcfile = 'D:/Testing/Anlyze2D/Create Model/src6.src'
statfile = 'D:/Testing/Anlyze2D/Create Model/stat6.stat'
velfile = 'D:/Testing/Anlyze2D/Create Model/vin6.vel2d'
veloutfile = 'D:/Testing/Anlyze2D/fwd/crt1.vel2d'
evtfile = 'D:/Testing/Anlyze2D/fwd/crt1.evt'
rayfile = 'D:/Testing/Anlyze2D/fwd/crt1.ray2d'
gr = 2
biter = 100
cacah = 1
pert = 0.3
# main = fwdModel(srcfile,statfile,velfile,evtfile,rayfile,veloutfile,gr,biter,cacah,pert)
