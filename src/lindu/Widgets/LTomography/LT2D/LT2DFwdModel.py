...
import lindu.Widgets.LTomography.LT2D.actFwdModel as fwd
import lindu.Widgets.LTomography.LT2D.display as disp
import lindu.Widgets.LTomography.LT2D.displayVel as dispV

from lindu.Widgets.LTomography.LT2D.thding import Worker, MessageBox
from lindu.Widgets.LTomography.LT2D.tictac import tic, tac
...
class MainWindow(QMainWindow):
    ...
    def executeClickButtonExecute(self, progress_callback):
        ...
        fwd.fwdModel(self.lineSource.text(),self.lineStat.text(),self.lineVel2d.text(),self.lineEvent.text(),
                     self.lineRay.text(),int(self.split.text()),int(self.biter.text()),int(self.cacah.text()),
                     progress_callback)
        elt = tac()
        return elt
