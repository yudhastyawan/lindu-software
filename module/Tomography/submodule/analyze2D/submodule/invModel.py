...
import module.Tomography.submodule.analyze2D.submodule.subroutine.actInvModel as invM
import module.Tomography.submodule.analyze2D.submodule.subroutine.display as disp
import module.Tomography.submodule.analyze2D.submodule.subroutine.displayVel as dispV
import module.Tomography.submodule.analyze2D.submodule.subroutine.displayPer as dispP
import module.Tomography.submodule.analyze2D.submodule.subroutine.displayRms as dispR
...
class MainWindow(QMainWindow):
    ...
    def setMainGroupBox(self):
        ...
        self.lineVel2d.setPlaceholderText('2D Velocity Init (*.vel2d) path')
        ...
        # -> settings to layout
        layoutInputData.addRow(self.lineEvent, buttonEvent)
        ...
        lb_noise = QLabel()
        lb_noise.setText('Noise')
        # lb_pert.setToolTip('Index of cluster to be relocated (0 = all)')
        self.noise = QLineEdit()
        self.noise.setText('0.3')

        # -> settings to layout
        layoutParameter.addRow(lb_normd, self.normd)
        layoutParameter.addRow(lb_gradd, self.gradd)
        layoutParameter.addRow(lb_iter, self.iter)
        ...
        layoutParameter.addRow(lb_noise, self.noise)
    ...
    def clickButtonEvent(self):
        open = QFileDialog()
        filepath = open.getOpenFileName(self, 'Open event file', '/', "Format File (*.evt)")
    ...
    def setMainTabWidget(self):
        ...
        perView = dispP.mainTabRayDisplay()
        self.mainTabWidget.addTab(perView, 'Pert Velocity')
        rmsView = dispR.mainTabRayDisplay()
        self.mainTabWidget.addTab(rmsView, 'RMS')
