...
class rayDisplay(FigureCanvas):
    def __init__(self,rgrid,rpath,vel,x,y,vmin,vmax,cmap,parent=None):
...
class mainRayDisplay(QWidget):
    def __init__(self, rayfile, velfile, parent = None):
        ...
        self.disp = rayDisplay(rgrid,rpath,vel,x,y,float(self.vmin.text()),float(self.vmax.text()),self.colormap.currentText())
    ...
    def view_click(self):
        ...
        self.disp = None
        self.tbar = None
        self.disp = rayDisplay(self.rgrid, self.rpath, self.vel, self.x, self.y, float(self.vmin.text()), float(self.vmax.text()),
                               cmap)
        self.tbar = NavigationToolbar(self.disp, self)
        self.mainLayout.addWidget(self.disp)
        self.mainLayout.addWidget(self.tbar)
...
class mainTabRayDisplay(QWidget):
    ...
    def clickButtonExecute(self):
        ...
        main = mainRayDisplay(self.lineRay.text(), self.lineVel2d.text())
        self.mainTab.addTab(main,str(fil1+'+'+fil2))
...
if __name__ == '__main__':
    ...
    main = mainRayDisplay(rayfile,velfile)
